...
public class Solution {
    public boolean isInterleave(String s1, String s2, String s3) {
        ...
        isInter[0][0] = true;
        // these initializations are not enough, should consider isInter[0][j], j > 1.
        // and isInter[i][0], i > 1.
        for (int j = 1; j < s2.length() + 1; ++j) {
            // isInter[0][j] = isInter[0][j - 1] && (s2.charAt(j) == s3.charAt(j));
            // above is wrong!!!!
        ...
        // isInter[1][0] = s1.charAt(0) == s3.charAt(0);
        for (int i = 1; i < s1.length() + 1; ++i) {
            for (int j = 1; j < s2.length() + 1; ++j) {
